...
public class ProductsDao extends BaseDao {
	...
	public List<ProductsDto> GetDataProductsPaginate(int id, int start, int totalPage) {
		...
		List<ProductsDto> listProductsbyID = _jbJdbcTemplate.query(sqlGetDatabyID.toString(), new ProductDtoMapper());
		List<ProductsDto> listProducts = new ArrayList<ProductsDto>();
		if(listProductsbyID.size() >0) {
			String sql = SqlProductsPaginate(id ,start, totalPage);
			listProducts=_jbJdbcTemplate.query(sql, new ProductDtoMapper());
		}
	...
	public List<ProductsDto> GetProductByID(long id) {
		...
		List<ProductsDto> listProduct=_jbJdbcTemplate.query(sql, new ProductDtoMapper());
		return  listProduct;
	}
	
	public ProductsDto FindProductByID(long id) {
		String sql = SqlProductByID(id);
		ProductsDto product=_jbJdbcTemplate.queryForObject(sql, new ProductDtoMapper());
		return  product;
	}
}
